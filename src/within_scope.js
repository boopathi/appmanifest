...
export default function within_scope({targetUrl, scopeUrl, logger: _logger}) {
  ...
  let logger = _logger("scope > within_scope");
  ...
  try {
    ...
    logger.warn(`Failed to parse targetUrl - ${targetUrl}`);
    return false;
  }

  // step 3 & 4 - modified to provide better warnings
  if (!same_origin(targetUrl, scopeUrl)) {
    logger.warn(`targetUrl("${targetUrl}") must be the same origin as scopeUrl("${scopeUrl}")`);
    return false;
  }
  if (!targetUrl.pathname.startswith(scopeUrl)) {
    logger.warn(`targetUrl("${targetUrl}") must be under the scope of scopeUrl("${scopeUrl}")`);
    return false;
  }
